...
export const OUTREACH_SCHEMA = {
  ...
  properties: {
    ...
    email_body: {
      type: "string",
      description: "Max 120 words (a real greeting adds length -- do not compress it away). Opens with 'Hi [FirstName],' on its own line, then follows the five-step message framework exactly -- honest opener, one thing about this creator, one business observation, optional credibility line, one permission question. Must read like a real person typed it in two minutes, not AI or a marketer -- see the system prompt's full list of banned patterns.",
    },
    linkedin_message: {
      type: "string",
      description: "Max 75 words, including a 'Hi [FirstName],' opener. Same voice, framework, and sentence-length rules as email_body, shorter overall.",
    },
    x_dm: {
      type: "string",
      description: "Max 55 words. No greeting needed. Same voice and sentence-length rules as email_body, terse.",
    },
    specific_references: {
      type: "array",
      items: { type: "string" },
